import { useGetQueue } from "@workspace/api-client-react";
import { Badge } from "@/components/ui/badge";
import { Button } from "@/components/ui/button";
import { format } from "date-fns";
import { vi } from "date-fns/locale";
import { Plane, CheckCircle2, Clock, MapPin, User, Video, List } from "lucide-react";
import { Skeleton } from "@/components/ui/skeleton";

export default function Queue() {
  const { data: queue, isLoading } = useGetQueue();

  if (isLoading) {
    return (
      <div className="container max-w-3xl mx-auto py-10 px-4 space-y-4">
        <Skeleton className="h-10 w-52" />
        <Skeleton className="h-24 w-full rounded-2xl" />
        <Skeleton className="h-24 w-full rounded-2xl" />
        <Skeleton className="h-24 w-full rounded-2xl" />
      </div>
    );
  }

  return (
    <div>
      {/* Page header */}
      <div className="bg-gradient-to-b from-slate-900 to-slate-800 text-white px-4 py-10">
        <div className="container max-w-3xl mx-auto">
          <div className="flex items-center gap-3 mb-2">
            <div className="p-2 bg-white/10 rounded-lg">
              <List className="w-5 h-5 text-sky-300" />
            </div>
            <h1 className="text-2xl font-bold">Hàng chờ công khai</h1>
          </div>
          <p className="text-slate-400 text-sm">Danh sách yêu cầu quay đang chờ và đã hoàn thành.</p>
        </div>
      </div>

      <div className="container max-w-3xl mx-auto py-6 px-4 space-y-3">
        {!queue?.length ? (
          <div className="flex flex-col items-center justify-center py-20 text-center">
            <div className="w-20 h-20 rounded-full bg-slate-100 flex items-center justify-center mb-4">
              <Plane className="w-9 h-9 text-slate-300" />
            </div>
            <h3 className="text-lg font-semibold text-slate-700 mb-1">Hàng chờ trống</h3>
            <p className="text-slate-400 text-sm">Chưa có chuyến bay nào được lên lịch.</p>
          </div>
        ) : (
          queue.map((entry, index) => {
            const isFilming = entry.status === "filming";
            const isCompleted = entry.status === "completed";

            return (
              <div
                key={entry.id}
                className={`relative rounded-2xl overflow-hidden border bg-white transition-all duration-200 ${
                  isFilming
                    ? "border-sky-300 shadow-lg shadow-sky-100"
                    : "border-slate-200 shadow-sm hover:shadow-md"
                }`}
              >
                {/* Filming glow strip */}
                {isFilming && (
                  <div className="absolute inset-0 bg-gradient-to-r from-sky-500/5 to-transparent pointer-events-none" />
                )}

                <div className="flex items-stretch">
                  {/* Number badge */}
                  <div className={`flex items-center justify-center w-16 shrink-0 text-xl font-bold ${
                    isFilming
                      ? "bg-gradient-to-b from-sky-500 to-blue-600 text-white"
                      : isCompleted
                      ? "bg-slate-100 text-slate-400"
                      : "bg-slate-50 text-slate-500"
                  }`}>
                    {isFilming ? (
                      <Plane className="w-6 h-6 animate-pulse" />
                    ) : (
                      `#${index + 1}`
                    )}
                  </div>

                  {/* Content */}
                  <div className="flex-1 p-4 min-w-0">
                    <div className="flex items-start justify-between gap-3 flex-wrap">
                      <div className="min-w-0">
                        <h3 className={`font-semibold leading-tight line-clamp-2 ${isCompleted ? "text-slate-500" : "text-slate-800"}`}>
                          {entry.locationName}
                        </h3>
                        <div className="flex flex-wrap items-center gap-x-3 gap-y-1 mt-1.5">
                          <span className="flex items-center text-xs text-slate-400">
                            <User className="w-3.5 h-3.5 mr-1" /> {entry.name}
                          </span>
                          <span className="flex items-center text-xs text-slate-400">
                            <MapPin className="w-3.5 h-3.5 mr-1" />
                            {entry.latitude.toFixed(4)}, {entry.longitude.toFixed(4)}
                          </span>
                        </div>
                      </div>

                      <div className="flex flex-col items-end gap-2 shrink-0">
                        {isFilming && (
                          <Badge className="bg-sky-500 hover:bg-sky-500 text-white text-xs px-2.5 py-1 shadow-sm">
                            <span className="inline-block w-1.5 h-1.5 rounded-full bg-white mr-1.5 ring-pulse" />
                            Đang quay
                          </Badge>
                        )}
                        {entry.status === "approved" && (
                          <Badge variant="secondary" className="text-xs px-2.5 py-1">
                            <Clock className="w-3 h-3 mr-1" /> Đang chờ
                          </Badge>
                        )}
                        {isCompleted && (
                          <Badge className="bg-emerald-100 text-emerald-700 hover:bg-emerald-100 border-none text-xs px-2.5 py-1">
                            <CheckCircle2 className="w-3 h-3 mr-1" /> Hoàn thành
                          </Badge>
                        )}

                        {entry.scheduledAt && !isCompleted && (
                          <p className="text-xs text-slate-500 font-medium">
                            {format(new Date(entry.scheduledAt), "dd/MM/yyyy HH:mm", { locale: vi })}
                          </p>
                        )}

                        {entry.videoUrl && (
                          <Button variant="outline" size="sm" asChild className="h-8 text-xs border-sky-200 text-sky-700 hover:bg-sky-50">
                            <a href={entry.videoUrl} target="_blank" rel="noopener noreferrer">
                              <Video className="w-3.5 h-3.5 mr-1.5" /> Xem video
                            </a>
                          </Button>
                        )}
                      </div>
                    </div>
                  </div>
                </div>
              </div>
            );
          })
        )}
      </div>
    </div>
  );
}
