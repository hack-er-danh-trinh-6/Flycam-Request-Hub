...
// ─── Child: exposes map.flyTo via a ref callback ─────────────────────────────
    ...
    click: (e) => {
      ...
      void (async () => {
        ...
        try {
          ...
          // We call parent via the callback bound in MapPicker
          window.__mapPickerPendingSelect = { lat, lng, name: data.display_name ?? `${lat.toFixed(5)}, ${lng.toFixed(5)}`, restricted };
        } catch {
          window.__mapPickerPendingSelect = { lat, lng, name: `${lat.toFixed(5)}, ${lng.toFixed(5)}`, restricted };
        }
      })();
    },
  });
...
declare global {
  interface Window {
    __mapPickerPendingSelect?: { lat: number; lng: number; name: string; restricted: boolean };
  }
}
...
export function MapPicker({ onLocationSelect }: MapPickerProps) {
  ...
  const handleSelect = useCallback((lat: number, lng: number) => {
    ...
    // Poll for async result from LocationMarker
    const check = () => {
      if (window.__mapPickerPendingSelect && window.__mapPickerPendingSelect.lat === lat) {
        const { lat: la, lng: lo, name, restricted } = window.__mapPickerPendingSelect;
        delete window.__mapPickerPendingSelect;
        onLocationSelect(la, lo, name, restricted);
      } else {
        setTimeout(check, 50);
      }
    };
  ...
  return (
    <div className="space-y-2">
      {/* Locate button — outside the map, no Leaflet DOM issues */}
      <div className="flex items-center gap-3">
        ...
        {myPos && !locating && (
          <span className="text-sm text-muted-foreground">
            Đã xác định: {myPos.lat.toFixed(4)}, {myPos.lng.toFixed(4)}
          </span>
        )}
      </div>
      ...
      <div className="relative h-[360px] w-full rounded-md border overflow-hidden" style={{ touchAction: "none" }}>
        ...
        >
          ...
          {/* Click to select */}
          <LocationMarker position={selectedPos} onSelect={handleSelect} noFlyZones={noFlyZones} />
        </MapContainer>
